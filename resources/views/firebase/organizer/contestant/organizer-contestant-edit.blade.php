...
<div class="contestant-setup-form-container">
        ...
        <form action="{{ url('update-contestant/'.$key) }}" method="POST">
            @csrf
            @method('PUT')
                    ...
                    <select name="event_name" id="event" class="form-control" required>
                        ...
                        @foreach($events as $eventId => $event)
                            <option value="{{ $eventId }}" 
                                {{ $editdata['event_name'] == $eventId ? 'selected' : '' }}>
                                {{ $event['ename'] }}
                            </option>
                        @endforeach
                    </select>
            ...
            <div class="form-group text-center ">
                <button type="button" class="btn-cancel" onclick="window.location.href='{{ route('contestatnant-list') }}'">Cancel</button>
